...
# 构建二进制文件
RUN GOPROXY=https://goproxy.cn,direct GOOS=${GO_OS} GOARCH=${GO_ARCH} go build -v -o checkwithdraw -ldflags '-s -w' cmd/main.go
...
COPY --chown=sato --from=build /usr/local/build/checkwithdraw /data/checkwithdraw
...
# 启动命令
CMD ["./checkwithdraw", "-config", "config.yaml"]